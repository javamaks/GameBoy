...
private:
    ...
    // Динамический контроллер памяти
    MemoryController* controller;

    // Регионы памяти
    vector<Byte> VRAM;     // $8000 - $9FFF, 8kB Видеопамять
    vector<Byte> OAM;      // $FE00 - $FEA0, OAM Спрайтовая память
    vector<Byte> WRAM;     // $C000 - $DFFF, 8kB Рабочая память
    vector<Byte> ZRAM;     // $FF80 - $FFFF, 128 байт ОЗУ
